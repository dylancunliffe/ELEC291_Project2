...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static void TurnMTR(uint8_t horizontal, uint8_t vertical, uint8_t eco) {

	// need to initialize the PWM generation for the timer pins
	...
	// Maybe have ECO as either 2 or 1?

	// TIM2_CH1 -> Motor1_1
	// TIM2_CH2 -> Motor1_2
	// 1_1 PWM, 1_2 GND for FWD
	...
	if(vertical > 0) {
		TIM2->CCR1 = vertical * eco; // don't exceed 999
		TIM2->CCR2 = 0;
	}
	else {
		TIM2->CCR1 = 0;
		TIM2->CCR2 = vertical * eco;
	}

	// HORZ MOVEMENT

	if(horizontal > 0) {
		TIM22->CCR1 = horizontal * eco;
		TIM22->CCR2 = 0;
	}
	else {
		TIM22->CCR1 = 0;
		TIM22->CCR2 = horizontal * eco;
	}
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, LED_L_Pin|LED_R_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : PB6 TOF_SDA_Pin */
  GPIO_InitStruct.Pin = GPIO_PIN_6|TOF_SDA_Pin;
